    ...
    if (ros_pc2->fields[3].name == "rgb") {
      ...
      for (size_t i = 0; i < ros_pc2->height * ros_pc2->width;
        ...
        o3d_pc.colors_.push_back(Eigen::Vector3d(
          (static_cast<int>((*ros_pc2_r))) / 255.0, (static_cast<int>((*ros_pc2_g))) / 255.0,
  ...
  std::string frame_id, int t_num_fields, ...)
{
  ...
  for (auto field_name = field_names.begin(); field_name != field_names.end(); ++field_name) {
    ...
    if (*field_name == "xyz") {
      for (auto i = 0; i < o3d_TensorList_points.GetShape()[0];
           i++, ++ros_pc2_x, ++ros_pc2_y, ++ros_pc2_z) {
      ...
      sensor_msgs::PointCloud2Iterator<uint8_t> ros_pc2_b(ros_pc2, "b");
      for (auto i = 0; i < o3d_TensorList_points.GetShape()[0];
           i++, ++ros_pc2_r, ++ros_pc2_g, ++ros_pc2_b) {
...
void ros_to_open3d(
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr & ros_pc2,
  open3d::t::geometry::PointCloud & o3d_tpc, bool skip_colors)
{
  sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_x(*ros_pc2, "x");
  sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_y(*ros_pc2, "y");
  sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_z(*ros_pc2, "z");
  ...
  for (int num_fields = 0; num_fields < static_cast<int>(ros_pc2->fields.size()); num_fields++) {
    if (ros_pc2->fields[num_fields].name == "x") {
      ...
      for (size_t i = 0; i < ros_pc2->height * ros_pc2->width;
           ++i, ++ros_pc2_r, ++ros_pc2_g, ++ros_pc2_b) {
        o3d_TensorList_colors.push_back(Eigen::Vector3d(
          (static_cast<int>((*ros_pc2_r))) / 255.0, (static_cast<int>((*ros_pc2_g))) / 255.0,
          (static_cast<int>((*ros_pc2_b))) / 255.0));
      }
      ...
      if (
        ros_pc2->fields[num_fields].datatype == sensor_msgs::msg::PointField::UINT8 ||
        ros_pc2->fields[num_fields].datatype == sensor_msgs::msg::PointField::INT8) {
        sensor_msgs::PointCloud2ConstIterator<uint8_t> ros_pc2_fx(
          *ros_pc2, ros_pc2->fields[num_fields].name);
        sensor_msgs::PointCloud2ConstIterator<uint8_t> ros_pc2_fy(
          *ros_pc2, ros_pc2->fields[num_fields].name);
        sensor_msgs::PointCloud2ConstIterator<uint8_t> ros_pc2_fz(
          *ros_pc2, ros_pc2->fields[num_fields].name);
        ...
        sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_fx(
          *ros_pc2, ros_pc2->fields[num_fields].name);
        sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_fy(
          *ros_pc2, ros_pc2->fields[num_fields].name);
        sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_fz(
          *ros_pc2, ros_pc2->fields[num_fields].name);
        std::vector<Eigen::Vector3d> o3d_TensorList_fields;
